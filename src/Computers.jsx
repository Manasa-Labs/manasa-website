import { Merged, PerspectiveCamera, RenderTexture, Text, useGLTF, useTexture } from '@react-three/drei'
...
export function ScreenText({ invert, x = 0, y = 1.2, ...props }) {
  ...
  return (
    <Screen {...props}>
      ...
      {showBSOD && activeTexture && activeTexture.image?.width > 0 ? (
        <>
          <color attach="background" args={['black']} />
          <mesh>
            <planeGeometry
              args={[
                2 * (activeTexture.image.width / activeTexture.image.height), // desired width
                9 * (activeTexture.image.height / activeTexture.image.width) // maintain aspect
              ]}
            />
        ...
        <>
          ...
          <Text font="/funnel-display-v2-latin-regular.woff" position={[x, y, 0]} ref={textRef} fontSize={4} letterSpacing={-0.1} color={!invert ? 'black' : '#35c19f'}>
            manasa
          </Text>
        </>
      )}
