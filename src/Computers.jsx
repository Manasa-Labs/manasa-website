...
export function ScreenText({ invert, x = 0, y = 1.2, ...props }) {
  ...
  if (showBSOD && activeTexture?.image?.width && activeTexture?.image?.height) {
    ...
    bsodWidth = baseHeight * textureAspect;
    bsodHeight = baseHeight;
  }
      ...
      <group onClick={triggerRipple}>
        {showBSOD && activeTexture?.image?.width > 0 ? (
          <>
            ...
            <mesh>
              <planeGeometry args={[bsodWidth, bsodHeight]} />
          ...
          <>
            <color
              attach="background"
              args={[invert ? "#101010" : "#35c19f"]}
            />
            <ambientLight intensity={0.5} />
            <directionalLight position={[10, 10, 5]} />
            <Text
              font="/funnel-display-v2-latin-regular.woff"
              position={[x, y, 0]}
              ref={textRef}
              fontSize={4}
              letterSpacing={-0.1}
              color={invert ? "#35c19f" : "#101010"}
            >
              manasa
            </Text>
            <animated.mesh scale={ripple.scale.to((s) => [s, s, 1])}>
